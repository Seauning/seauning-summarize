/*! For license information please see tutorial.0085805d53a665cf3a7e.js.LICENSE.txt */
...
for (var w = g.pattern || g, k = r.next, S = l; k !== t.tail && !(d && S >= d.reach); S += k.value.length, 
k = k.next) {
var _ = k.value;
...
for (L += k.value.length; x >= L; ) L += (k = k.next).value.length;
if (S = L -= k.value.length, k.value instanceof i) continue;
for (var O = k; O !== t.tail && (L < F || "string" == typeof O.value); O = O.next) T++, 
L += O.value.length;
T--, _ = e.slice(S, L), A.index -= S;
} else if (!(A = s(w, 0, _, b))) continue;
x = A.index;
var C = A[0], I = _.slice(0, x), N = _.slice(x + C.length), R = S + _.length;
d && R > d.reach && (d.reach = R);
var P = k.prev;
if (I && (P = c(t, P, I), S += I.length), u(t, P, T), k = c(t, P, new i(p, m ? a.tokenize(C, m) : C, y, C)), 
N && c(t, k, N), T > 1) {
var D = {
cause: p + "," + f,
reach: R
};
o(e, t, n, k.prev, S, D), d && D.reach > d.reach && (d.reach = D.reach);
}
}
}
}
...
let f, g, m, b = n.classList.contains("language-javascript"), v = n.classList.contains("language-markup"), y = +e.getAttribute("data-trusted"), E = !+e.getAttribute("data-no-strict") && b ? '"use strict";' : "", w = 'globalThis.__codeBoxId = "'.concat(e.id, '";'), k = "".concat(E, "\n\n").concat(d);
d = "".concat(E).concat(w, "\n\n").concat(d);
let S = !0;
if (!b && !v) return;
...
0 === ie && le(w)); else r = o;
if (r !== a) if (32 === t.charCodeAt(Q) ? (i = k, Q++) : (i = a, 0 === ie && le(S)), 
i === a && (i = p), i !== a) {
if (s = [], (l = fe()) !== a) for (;l !== a; ) s.push(l), l = fe(); else s = o;
s !== a ? (ee = e, e = n = _(r, s)) : (Q = e, e = o);
...
//# sourceMappingURL=tutorial.0085805d53a665cf3a7e.js.map